...
reKey="$1"
mkdir -p /usr/local/bin/client-files/.ssh 2> /dev/null
...
if [ -e /usr/local/bin/blueskyd ] && [ "$reKey" == "" ]; then
	echo "This server has already been configured.  Please use --client or --admin to re-key the client apps."
	echo "If you are trying to set up the server again, please delete /usr/local/bin/blueskyd* and try again."
	exit 1
fi

if [ "$reKey" != "--admin" ]; then
	# make blueskyclient pair - used for encrypting uploaded SSH keys to the server for clients
	openssl req -x509 -nodes -days 100000 -newkey rsa:2048 -keyout /usr/local/bin/blueskyclient.key -out /usr/local/bin/client-files/blueskyclient.pub -subj '/'
fi

if [ "$reKey" != "--client" ]; then
	# make blueskyadmin pair - used for encrypting uploaded SSH keys to the server for admins
	openssl req -x509 -nodes -days 100000 -newkey rsa:2048 -keyout /usr/local/bin/blueskyadmin.key -out /usr/local/bin/client-files/blueskyadmin.pub -subj '/'
fi
...
if [ "$reKey" == "" ]; then
	...
	ssh-keygen -q -t rsa -N '' -f /usr/local/bin/blueskyd -C "$hostName"
	chown www-data /usr/local/bin/blueskyd
	echo command=\"/var/bluesky/.ssh/wrapper.sh\",no-port-forwarding,no-X11-forwarding,no-agent-forwarding,no-pty `cat /usr/local/bin/blueskyd.pub` > /usr/local/bin/client-files/.ssh/authorized_keys
...
</plist>" > /usr/local/bin/client-files/server.plist
fi
